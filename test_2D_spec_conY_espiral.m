...
% Basado en test_2D_spec_espiral (espiral de Arquímedes)
...
%% Trayectoria espiral k-space: Espiral de Arquímedes
% A diferencia de la versión con VDS, aquí se usa la espiral de Arquímedes
% (espiralArquimedes), que es una trayectoria de densidad uniforme sin
% parámetros Ninter ni Fcoeff. La trayectoria se invierte (end:-1:1)
% igual que en test_2D_spec_espiral para recorrer el k-space de fuera
% hacia dentro.

[kx1, ky1] = espiralArquimedes(0, 0.1, 5, 1000);
kx = kx1(end:-1:1); % k-space espacial X [1/cm]
ky = ky1(end:-1:1); % k-space espacial Y [1/cm]
g = k2g([kx; ky], FOVx);
gx = g(1,:); gy = g(2,:);
Nt = length(kx);
...
dtarget(fatval, dfatX, dfatY) = flip * pi/180; %en las frecuencias de grasa (fatval),
...
rango = [0, flip*pi/180];
% dibujar roi con x,y y frecuencia
figure; im(permute(dtarget([fatval watval],:,:),[2 3 1]), rango);
colormap(gca,'default')
figure; im(abs(permute(mopt([fatval watval],:,:).*roi([fatval watval],:,:),[2 3 1])), rango);
colormap(gca,'default')
...
% Promedio en las frecuencias correspondientes a la grasa
mopt_grasa = squeeze(mean(abs(mopt(fatval,:,:)), 1));
dtarget_grasa = squeeze(mean(dtarget(fatval,:,:), 1));

% Escala común para ambas imágenes
clim_xy = [0 max([dtarget_grasa(:); mopt_grasa(:)])];

figure('Name','Imagen espacial 2D - banda de grasa', ...
       'Position',[100 100 1100 420]);

subplot(1,2,1);
imagesc(x, y, dtarget_grasa.', clim_xy);
axis xy;
axis image;
colormap(gca,'parula');
h = colorbar;
...
subplot(1,2,2);
imagesc(x, y, mopt_grasa.', clim_xy);
axis xy;
axis image;
colormap(gca,'parula');
h = colorbar;
...
for k = 1:4
    ...
    subplot(4, 2, (k-1)*2 + 2);
    imagesc(x, frange, abs(mopt(:,:,iy)) .* roi(:,:,iy), [0 flip*pi/180]);
    xlabel('X [cm]'); ylabel('Frec. [Hz]');
    colorbar;
    title(sprintf('Magnetización (ROI) | Y = %.1f cm', y(iy)));
end
...
% Buscar los índices de frecuencia de interés
idx_freqs = zeros(1,length(freqs_interes));
for k = 1:length(freqs_interes)
    [~, idx_freqs(k)] = min(abs(frange - freqs_interes(k)));
end

% Escala común para TODAS las imágenes
max_target = 0;
max_mopt   = 0;

for k = 1:length(idx_freqs)
    if_k = idx_freqs(k);
    max_target = max(max_target, max(max(squeeze(dtarget(if_k,:,:)))));
    max_mopt   = max(max_mopt,   max(max(squeeze(abs(mopt(if_k,:,:))))));
end

clim_freq = [0 max(max_target, max_mopt)];

for k = 1:4
    if_k = idx_freqs(k);

    subplot(4, 2, (k-1)*2 + 1);
    imagesc(x, y, squeeze(dtarget(if_k,:,:)).', clim_freq);
    axis xy;
    axis image;
    colormap(gca,'parula');
    h = colorbar;
    ...
    xlabel('X [cm]');
    ylabel('Y [cm]');
    title(sprintf('Target | %s | f=%.0f Hz', labels_freqs{k}, frange(if_k)));

    subplot(4, 2, (k-1)*2 + 2);
    imagesc(x, y, squeeze(abs(mopt(if_k,:,:))).', clim_freq);
    axis xy;
    axis image;
    colormap(gca,'parula');
    h = colorbar;
    ...
    xlabel('X [cm]');
    ylabel('Y [cm]');
    title(sprintf('Magnetización | %s | f=%.0f Hz', labels_freqs{k}, frange(if_k)));
end
...
% En el punto de agua debería ser bajo en todas las frecuencias.
