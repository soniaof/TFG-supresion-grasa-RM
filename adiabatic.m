...
if (blochsim)
	...
	if (exist('blochCim'))
      ...
      sens = 1;
	  mode = 0;
      for ii=1:length(df)               % modified SNW 160326 to include Bloch Sim C file
	    [mx(ii),my(ii),mz(ii)] = blochCim(b1.*exp(i*phase),gr,tp,t1,t2,df(ii),dp,mode,sens);
      end
	  figure(2); 
	  ...
	  disp('blochCim.m not found.  No Bloch simulation.');
	end;
end;