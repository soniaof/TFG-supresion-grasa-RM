...
%     title(sprintf('Target | flip=%d', flips(i)));
% 
%     % % Magnetización Bloch
%     % subplot(4, 3, (i - 1) * 3 + 2);
...
for i = 1:length(flips)
    ...
    NRMSE_sta = norm(abs(mopt_i).*roi_i - dtarget_i) / norm(dtarget_i);
    NRMSE_val = norm(abs(Mxyspec).*roi_i - dtarget_i) / norm(dtarget_i);
    fprintf('flip=%d | max(abs(bopt))=%.4f | norm(bopt)=%.4f | NRMSE=%.4f\n', flips(i), max(abs(bopt_i)), norm(bopt_i), NRMSE_val);
    ...
    % Magnetización STA
    subplot(4, 2, (i - 1) * 2 + 2);
    imagesc(x, frange, abs(mopt_i).*roi_i, [0 flips(i) * pi / 180]);
    xlabel('X [cm]'); ylabel('Frec. [Hz]'); colorbar;
    title(sprintf('Magnetización STA | ||b||=%.3f | NRMSE=%.3f', norm(bopt_i), NRMSE_sta));
    ...
    % xlabel('X [cm]'); ylabel('Frec. [Hz]'); colorbar;
    % NRMSE_val = norm(abs(Mxyspec).*roi_i - dtarget_i) / norm(dtarget_i);  % Calcular NRMSE
    % title(sprintf('Magnetización Bloch | ||b||=%.3f | NRMSE=%.3f', norm(bopt_i), NRMSE_val));
end
