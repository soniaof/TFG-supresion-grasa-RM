...
% Añadir las entradas a la función vds.m para los gradientes del sistema
% Sola
...
% Figura del pulso RF y trayectorias en k-space
figure('Name','VDS - Pulso RF y trayectoria k-space');
...
%% Guardar resultados VDS para la figura conjunta final

casos(1).nombre = 'a) VDS';
casos(1).x = x;
casos(1).frange = frange;
casos(1).target = dtarget;
casos(1).roi_resultado = abs(mopt).*roi;
casos(1).nrmse = nrmse_opt;
casos(1).flip = flip;


%% Figura individual VDS sin la columna de magnetización completa

figure('Name','VDS - Target y ROI','Position',[100 100 800 400]);
...
subplot(1,2,2);
imagesc(x,frange,abs(mopt).*roi,[0 (flip*pi/180)]);
axis xy;
xlabel('X [cm]');
ylabel('frec. [Hz]');
h = colorbar;
ylabel(h,'[rad]');
title(sprintf('VDS - ROI, NRMSE = %4.3f',nrmse_opt));
%% Próximo pasos
...
% gradientes del espiral invertidos y convertidos a G/cm
gxsim = real(g_sp(end:-1:1)) * 0.1;
gysim = imag(g_sp(end:-1:1)) * 0.1;
...
%% 3. Jugar con vds.m para crear espirales más dinámicos

Ninters = [1, 2, 1, 2];
Fcoeffs = [4, 4, 6, 6];
% guardar resultados de cada caso
mopt_casos  = cell(4,1);
nrmse_casos = zeros(4,1);
norm_casos  = zeros(4,1);
dur_casos   = zeros(4,1);
for i = 1:length(Ninters)
    Ninter_i = Ninters(i);
    Fcoeff_i = Fcoeffs(i);
    [k_sp_i, ~, ~, time_i, ~, ~] = vds(Sys.Smax*100, Sys.Gmax*0.1, Sys.dt, Ninter_i, Fcoeff_i, rmax);
    dur_casos(i) = time_i(end)*1e3;
    fprintf('Caso %d: Ninter=%d, Fcoeff=%d, duracion=%.2f ms\n', i, Ninter_i, Fcoeff_i, dur_casos(i));
    Nt_i = length(k_sp_i);
    kx_i = real(k_sp_i(end:-1:1));
    kt_i = time_i(end:-1:1);
    [F,X] = ndgrid(frange, x);
    r_spsp_i = [F(:) X(:)];
    k_spsp_i = [kt_i; kx_i];
    A_i = (1i*2*pi*4258*m0*Sys.dt) * exp(1i*2*pi*(r_spsp_i*k_spsp_i));
    bopt_i = pinv(A_i'*diag(roi(:))*A_i + beta*eye(Nt_i)) * A_i'*diag(roi(:))*dtarget(:);
    mopt_casos{i}  = reshape(A_i*bopt_i, [Nf Nx]);
    nrmse_casos(i) = func.nrmse(mopt_casos{i}, dtarget, roi);
    norm_casos(i)  = norm(bopt_i);
    
    fprintf('Ninter=%d, Fcoeff=%d | max(abs(bopt))=%.4f | norm(bopt)=%.4f | NRMSE=%.4f\n', Ninter_i, Fcoeff_i, max(abs(bopt_i)), norm_casos(i), nrmse_casos(i));

end

close all;
figure('Position',[100 100 1400 900]);
sgtitle('Efecto de Ninter y Fcoeff en el diseño del pulso SPSP');
for i = 1:length(Ninters)
    subplot(4, 3, (i-1)*3 + 1);
    imagesc(x, frange, dtarget, [0 (flip*pi/180)]);
    xlabel('X [cm]'); ylabel('Frec. [Hz]'); colorbar;
    title(sprintf('Target | Ninter=%d | Fcoeff=%d', Ninters(i), Fcoeffs(i)));

    subplot(4, 3, (i-1)*3 + 2);
    imagesc(x, frange, abs(mopt_casos{i}), [0 (flip*pi/180)]);
    xlabel('X [cm]'); ylabel('Frec. [Hz]'); colorbar;
    title(sprintf('Magnetización | dur=%.1fms', dur_casos(i)));

    subplot(4, 3, (i-1)*3 + 3);
    imagesc(x, frange, abs(mopt_casos{i}).*roi, [0 (flip*pi/180)]);
    xlabel('X [cm]'); ylabel('Frec. [Hz]'); colorbar;
    title(sprintf('ROI | NRMSE=%.3f | ||b||=%.3f', nrmse_casos(i), norm_casos(i)));
end

% En este apartado se estudia el efecto de la trayectoria espiral en el
% diseño del pulso espectral-espacial.
%
% Se prueban 4 casos combinando Ninter (número de interleaves) y Fcoeff
% (coeficiente del FOV espectral):
%   - Ninter=1, Fcoeff=4 (caso base)
%   - Ninter=2, Fcoeff=4 (más vueltas)
%   - Ninter=1, Fcoeff=6 (mayor FOV espectral)
%   - Ninter=2, Fcoeff=6 (más vueltas y mayor FOV espectral)
%
% Para cada caso se recalcula la trayectoria con vds.m, la matriz del
% sistema A y el pulso óptimo. La magnetización se calcula con la
% aproximación lineal m=A*b (small tip angle).
%
% Resultados esperados:
% - Mayor Ninter: trayectoria más densa, mejor selectividad espacial
%   pero pulso más largo.
% - Mayor Fcoeff: mayor FOV espectral, mejor separación entre grasa y agua
%   pero también pulso más largo.
% - El NRMSE y la norma del pulso varían según la complejidad de la
%   trayectoria.


%% 3 (2). Jugar con vds.m para crear espirales más dinámicos

% Parámetros modificados
Ninter_new = 1;  % Ninter = 1
Fcoeff_new = 5;  % Fcoeff = 5
beta_values = [0.5, 1.5^-1];  % Valores de beta

% Guardar resultados para los dos valores de beta
mopt_casos_new  = cell(2,1);
nrmse_casos_new = zeros(2,1);
norm_casos_new  = zeros(2,1);
dur_casos_new   = zeros(2,1);

% Generar resultados para los dos valores de beta
for j = 1:length(beta_values)
    beta = beta_values(j);
    
    [k_sp_new, ~, ~, time_new, ~, ~] = vds(Sys.Smax*100, Sys.Gmax*0.1, Sys.dt, Ninter_new, Fcoeff_new, rmax);
    dur_casos_new(j) = time_new(end)*1e3;  % Duración en milisegundos
    fprintf('Caso con beta = %.2f: Ninter=%d, Fcoeff=%d, duracion=%.2f ms\n', beta, Ninter_new, Fcoeff_new, dur_casos_new(j));
    
    Nt_new = length(k_sp_new);
    kx_new = real(k_sp_new(end:-1:1));
    kt_new = time_new(end:-1:1);
    [F,X] = ndgrid(frange, x);
    r_spsp_new = [F(:) X(:)];
    k_spsp_new = [kt_new; kx_new];
    A_new = (1i*2*pi*4258*m0*Sys.dt) * exp(1i*2*pi*(r_spsp_new*k_spsp_new));
    
    % Recalcular el pulso óptimo
    bopt_new = pinv(A_new'*diag(roi(:))*A_new + beta*eye(Nt_new)) * A_new'*diag(roi(:))*dtarget(:);
    mopt_casos_new{j}  = reshape(A_new*bopt_new, [Nf Nx]);
    nrmse_casos_new(j) = func.nrmse(mopt_casos_new{j}, dtarget, roi);
    norm_casos_new(j)  = norm(bopt_new);
    
    fprintf('beta=%.4f, Ninter=%d, Fcoeff=%d | max(abs(bopt))=%.4f | norm(bopt)=%.4f | NRMSE=%.4f\n', beta, Ninter_new, Fcoeff_new, max(abs(bopt_new)), norm_casos_new(j), nrmse_casos_new(j));

end

% Crear la figura con los resultados para ambos valores de beta
close all;
figure('Position',[100 100 1400 900]);
sgtitle('Efecto de Ninter, Fcoeff y beta en el diseño del pulso SPSP');
...
for j = 1:length(beta_values)
    ...
    imagesc(x, frange, dtarget, [0 (flip*pi/180)]);
    xlabel('X [cm]'); ylabel('Frec. [Hz]'); colorbar;
    title(sprintf('Target | Ninter=%d | Fcoeff=%d | beta=%.2f', Ninter_new, Fcoeff_new, beta_values(j)));

    % Subgráfico 2: Magnetización resultante
    subplot(2, 3, (j-1)*3 + 2);
    imagesc(x, frange, abs(mopt_casos_new{j}), [0 (flip*pi/180)]);
    xlabel('X [cm]'); ylabel('Frec. [Hz]'); colorbar;
    title(sprintf('Magnetización | dur=%.1fms', dur_casos_new(j)));

    % Subgráfico 3: ROI con NRMSE y ||b||
    subplot(2, 3, (j-1)*3 + 3);
    imagesc(x, frange, abs(mopt_casos_new{j}).*roi, [0 (flip*pi/180)]);
    xlabel('X [cm]'); ylabel('Frec. [Hz]'); colorbar;
    title(sprintf('ROI | NRMSE=%.3f | ||b||=%.3f', nrmse_casos_new(j), norm_casos_new(j)));
end
